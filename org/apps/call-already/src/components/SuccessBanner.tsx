...
export const SuccessBanner: React.FC<SuccessBannerProps> = ({message, onClose}) => {
  ...
  return (
    <div style={{border: `1px solid ${color}`, padding: "1em", borderRadius: "3px", margin: "0 auto 0 auto", boxShadow: `3px 3px ${color}`}}>
      <div style={{display: "flex", gap: "0.5em", alignItems: "center"}}>
        <CloseButton style={{color: `${color}`}} mr={-6} onClick={() => onClose()} />
        <Text style={{color: `${color}`, textAlign: "center"}} fz="sm" fw={500}>
          {message}
        </Text>
      </div>
    </div>
